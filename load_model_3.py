...
# Tokenizer (assuming you have already defined this)
...
def make_inference_models():
    ...
    encoder_inputs = model_2.input[0]  # This is 'encoder_inputs'
    # Extracting the output of the encoder LSTM layer (named 'encoder_lstm')
    encoder_outputs, state_h_enc, state_c_enc = model_2.get_layer('encoder_lstm').output
    encoder_states = [state_h_enc, state_c_enc]

    # The encoder model is defined to take the encoder inputs and output the states
    encoder_model = tf.keras.models.Model(inputs=encoder_inputs, outputs=encoder_states, name='encoder_inference_model')

    # Decoder Inference Model
    decoder_inputs = model_2.input[1]  # This is 'decoder_inputs'
    # Creating new input layers to inject the encoder states into the decoder
    decoder_state_input_h = tf.keras.layers.Input(shape=(200,), name='decoder_state_input_h')
    decoder_state_input_c = tf.keras.layers.Input(shape=(200,), name='decoder_state_input_c')
    decoder_states_inputs = [decoder_state_input_h, decoder_state_input_c]

    # Reuse the decoder embedding layer (named 'decoder_embedding')
    decoder_embedding_layer = model_2.get_layer('decoder_embedding')
    decoder_embedding = decoder_embedding_layer(decoder_inputs)

    # Reuse the decoder LSTM layer (named 'decoder_lstm')
    decoder_lstm_layer = model_2.get_layer('decoder_lstm')
    decoder_outputs, state_h_dec, state_c_dec = decoder_lstm_layer(decoder_embedding, initial_state=decoder_states_inputs)
    decoder_states = [state_h_dec, state_c_dec]

    # Reuse the decoder dense layer (named 'decoder_dense')
    decoder_dense_layer = model_2.get_layer('decoder_dense')
    decoder_outputs = decoder_dense_layer(decoder_outputs)

    # The decoder model is defined to take the decoder inputs and the initial states, and output the decoder outputs and states
    decoder_model = tf.keras.models.Model(inputs=[decoder_inputs] + decoder_states_inputs,
